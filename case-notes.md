...
| 4 | I found that the 15:14 problem was caused by `amendment-314.sh`, which started CPU-hungry jobs every day. The crontab line was: `14 15 * * * $HOME/enginehouse/machinery/amendment-314.sh # Dispatch Board Amendment No. 314 — by order of [███]`. The paper's signature line was `#                         By order of [███]`. | `top`, `ps`, `crontab -l`, and `cat ~/enginehouse/machinery/amendment-314.sh` | I think this was meant to disrupt the Demonstration without actually damaging anything. It was carefully scheduled, lasted only 90 seconds, and cleaned itself up, so it seems more like someone was trying to make a point than destroy the system. |
| 5 | The two measured rows matched, but the two computed rows in Table IX did not match the fair copy. | Compared `output-ledger.txt`, `~/.ledger-annex`, and `working-papers-tableIX.txt`. | I think the problem is with how the computed values were produced or recorded, because the measured values stayed the same and my arithmetic matched the fair copy. |
